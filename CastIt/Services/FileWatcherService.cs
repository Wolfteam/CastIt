...
namespace CastIt.Services
{
    public class FileWatcherService : IFileWatcherService
    {
        ...
        public FileWatcherService(IMvxLogProvider logProvider)
        {
            _logger = logProvider.GetLogFor<FileWatcherService>();
        }
        ...
        public void StopListening()
        {
            foreach (var (path, watcher) in _watchers)
            {
                _logger.Info($"{nameof(StopListening)}: Stopping the watcher for path = {path}");
                StopListening(watcher);
            }
            _watchers.Clear();
        }
        ...
        {
            _logger.Info($"{nameof(StopListening)}: Stopping watcher for path = {watcher.Path}");
        ...
        {
            _logger.Info($"{nameof(WatchPath)}: Starting the watcher for path = {path}");
            if (_watchers.ContainsKey(path))
            {
        ...
        {
            _logger.Info($"{nameof(OnChanged)}: File: " + e.FullPath + " " + e.ChangeType);
            switch (e.ChangeType)
            {
        ...
        {
            _logger.Info($"{nameof(OnRenamed)}: File: {e.OldFullPath} renamed to {e.FullPath}");
